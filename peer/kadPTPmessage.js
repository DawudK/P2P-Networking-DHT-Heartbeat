...
module.exports = {
  ...
  init: function (ver, msgType, peerTable) {
    ...
    storeBitPacket(this.message, version * 1, 0, 5);

    // Message type
    storeBitPacket(this.message, messageType, 5, 8); // start at bit 5

    // Number of peers
    storeBitPacket(this.message, noOfPeers, 13, 5);

    // Sender name size
    storeBitPacket(this.message, senderName.length, 18, 14);
    let byteMarker = 4;
